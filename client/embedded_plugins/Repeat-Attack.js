...
//@ts-ignore
import {
    PlanetType, SpaceType, PlanetTypeNames //@ts-ignore
...
import {
    html, render, useEffect, useState, useLayoutEffect //@ts-ignore
...
const sendSilverStatuses = ['Do not send', // 0
    'Upgrade first', // 1
    'Send all' // 2
];
...
const getPlanetString = locationId => {
    ...
    let type = 'P';
    if (planet.planetType == PlanetType.SILVER_MINE) type = 'A'; else if (planet.planetType == PlanetType.RUINS) type = 'F'; else if (planet.planetType == PlanetType.TRADING_POST) type = 'STR'; else if (planet.planetType == PlanetType.SILVER_BANK) type = 'Q';
...
const getPlanetMaxRank = planet => {
    ...
    if (planet.spaceType === SpaceType.NEBULA) return 3; else if (planet.spaceType === SpaceType.SPACE) return 4; else return 5;
};
...
function unconfirmedDepartures(planet) {
    return planet.transactions?.getTransactions(isUnconfirmedMoveTx) //@ts-ignore
...
class Repeater {
    ...
    addAttack() {
        ...
        attack.pcRemain = attack.pcTrigger <= attack.pcRemain ? parseInt(attack.pcTrigger / 2) : attack.pcRemain; // Filter out existing attacks from this source, then add new attack at top
...
function Attack({
    ...
    <div style=${ActionEntry}>
      <button style=${{
            border: 'none',
            margin: 'none'
        }} onClick=${onToggleActive}>${attack.active ? '▶️' : '⏸️'}</button>
      <span>
        <span style=${{
            ...Margin_3L_3R
        }}>
          <span style=${{
            ...Clickable
        }} onClick=${() => centerPlanet(attack.sourceId)}>${finalSrc}</span>
          <span style=${{
            ...Margin_3L_3R
        }}>-></span>
          <span style=${{
            ...Clickable
        }} onClick=${() => centerPlanet(attack.targetId)}>${finalTarget}</span>
        </span>
        <span style=${{
            ...Margin_3L_3R
        }}>
...
function AddAttack({
    ...
    const toggleSendSilver = () => setCurrentAttack('sendSilverStatus', toggleSilverStatus(getCurrentAttack('sendSilverStatus')));
    ...
    <div style=${{
      ...
      <div style=${{
        ...
        Choose when to send silver: <button style=${{
            width: 150,
            height: 23,
            fontSize: '90%'
...
const drawHighlights = plugin => {
    ...
    attacksSelectedIsSource.forEach(a => a.active ? drawHighlight(a.targetId, `rgba(255, 80, 80, 0.6)`, 23000, 8, 0.55) : drawHighlight(a.targetId, `rgba(180, 140, 40, 0.6)`, 23000, 6, 0.3));
    drawHighlight(selectedPlanetId, `rgba(80, 80, 255, 0.7)`, -12000, 6, 0.7);
    attacksSelectedIsTarget.forEach(a => a.active ? drawHighlight(a.sourceId, `rgba(80, 255, 80, 0.5)`, 7000, 4, 0.8) : drawHighlight(a.sourceId, `rgba(140, 180, 40, 0.5)`, 7000, 3, 0.4));
};
